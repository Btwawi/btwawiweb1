...
export function SpeakersSection({ data }: SpeakersSectionProps) {
  const settings = {
    ...
    appendDots: (dots: React.ReactNode) => (
      <div className="flex justify-center gap-3 mt-8">{dots}</div>
    ),
  };
            ...
            {data.map((speaker, index) => (
              <div key={index} className="px-3">
                <div className="w-[341px] h-[422px] relative mx-auto">
                  {/* Background */}
                  <div
                    className={`w-full h-[420px] ${speaker.bgColor} rounded-t-lg`}
                  />
                  ...
                  <div className="absolute bottom-0 w-full h-[136px] bg-btwawi-yellow rounded-b-lg flex flex-col items-center justify-center text-center px-4">
                    <h3
                      className={`text-btwawi-blue font-aeonik text-3xl lg:text-4xl font-normal tracking-[-1.8px] mb-2 ${
                        speaker.nameStyle || "leading-[60px]"
                      }`}
                    >
                    ...
                    <p
                      className={`text-btwawi-blue font-aeonik text-lg lg:text-[23px] font-normal tracking-[-1.15px] max-w-[250px] ${
                        speaker.roleStyle || "leading-[38.37px]"
                      }`}
                    >
                      {speaker.role}
                    </p>
