...
import { BoardMemberCategoryItem } from "../../utils/dataTypes";
...
interface BoardMemberCategoryData {
  data: BoardMemberCategoryItem[];
  meta: Meta;
  links: Links;
}

export interface BoardMemberCategoryApiResponse {
  status: string;
  statusCode: number;
  message: string;
  data: BoardMemberCategoryData;
}
...
function BoardMemberCategoryMain() {
  const [viewModal, setViewModal] = useState(false);
  const [editModal, setEditModal] = useState(false);
  const [deleteModal, setDeleteModal] = useState(false);
  const [selectedCategory, setSelectedCategory] =
    useState<BoardMemberCategoryItem | null>(null);
  ...
  const handleView = (category: BoardMemberCategoryItem) => {
    setSelectedCategory(category);
    setViewModal(true);
  };

  const handleDelete = async (category: BoardMemberCategoryItem) => {
    try {
  ...
  const openEditModal = (category: BoardMemberCategoryItem) => {
    setSelectedCategory(category);
    setEditForm({
      name: category.name,
      ...
      <div className="mt-5 intro-y box">
        <div className="flex flex-col items-center p-5 border-b sm:flex-row border-slate-200/60">
          ...
          <div className="w-full mt-3 sm:w-auto sm:ml-auto sm:mt-0">
            <Button
              onClick={() => navigate("/create-board-member-categories")}
...
export default BoardMemberCategoryMain;
